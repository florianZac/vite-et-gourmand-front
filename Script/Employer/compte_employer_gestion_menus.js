...
export function initGestionMenusEmployerPage() {
  ...
  function renderMenus(menus) {
    ...
    menus.forEach(menu => {
      const themeLabel = menu.theme ? sanitizeHtml(menu.theme.titre) : " ";
      const regimeLabel = menu.regime ? sanitizeHtml(menu.regime.libelle) : " ";
      ...
      row.innerHTML = `
        <div>
          ...
          <small class="text-muted">${themeLabel} · ${regimeLabel} · Min. ${sanitizeHtml(menu.nombre_personne_minimum || 1)} pers. · ${sanitizeHtml(menu.prix_par_personne || 0)}€/pers</small><br>
          <small>${nbPlats} plat(s) — ${enStock ? '<span style="color:green">En stock</span>' : '<span style="color:red">Rupture</span>'} · Qté: ${sanitizeHtml(menu.quantite_restante || 0)}</small>
        </div>
  ...
  function openEditForm(menuId) {
    ...
    menuFormTitle.textContent = `Modifier : ${sanitizeHtml(menu.titre)}`;
  ...
  function applyFilters() {
    ...
    if (searchInput) {
      search = searchInput ? sanitizeInput(searchInput.value.toLowerCase().trim()) : "";
    }

    if (filterStatus) {
      status = filterStatus ? sanitizeInput(filterStatus.value) : "";
    }
