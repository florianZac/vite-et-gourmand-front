...
export function initCommandeClientPage() {
  ...
  // URL de récupération de commande de l'API Symfony
  const apiCommandeClient = `${API_URL}/api/commander`;

  // URL de base de l'API Symfony
  const apiAnnulation_commande = `${API_URL}/orders/${orderId}/cancel`;
    ...
    cancelButtons.forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          const response = await fetch(apiAnnulation_commande, {
            method: 'PATCH',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
